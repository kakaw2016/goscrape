...
func (stock *product) collectedata(data map[string][]string) {
	...
	for key, value := range data {
		...
		if key == "parent_author" && len(data["parent_author"]) != 0 {
			...
			reg1 := regexp.MustCompile("((freed99)|(hiveborgminer)|(ydaiznfts)|(bokica80)|(spectrumecons)|(bechibenner)|(phortun)|(pundito)|(oadissin))")
		...
		if key == "title" && len(data["title"]) != 0 {
			...
			if _, ok := data["title"]; ok {
				...
				reg1 := regexp.MustCompile("(.*(hh guild).*|.*(weed).*|.*(gimmi).*|.*(mage).*|.*(art).*|.*(lolz).*|.*(powerup).*|.*(psyber).*|.*(risingstar).*|.*(virtualgrowth).*|.*(star).*|.*(rising).*|.*(crop).*|.*(woo).*|.*(alive).*|.*(chess).*|.*(phortun).*|.*(woogame).*|.*(euro).*|.*(goldquizblog).*|.*(hashkings).*)")
...
Delegation - Raffles - Giveaways: 10+ Updated Contests // Directory #️⃣ 100

splinterlands spt leofinance indiaunited play2earn thgaming play2own oneup neoxian pgm pimp proofofbrain waiv tribes archon cent vyb ctp
